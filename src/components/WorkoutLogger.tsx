...
export const WorkoutLogger: React.FC<WorkoutLoggerProps> = ({
  ...
  return (
    <div className="space-y-4 pb-12 sm:pb-6">
      {/* Top Header Card */}
      <div className="bg-[#FAF8F5] border border-[#E3DCD1] p-4 sm:p-5 rounded-xl shadow-xs space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#781D2E] animate-ping" />
              <span className="text-[10px] uppercase font-mono-num font-bold text-[#781D2E] tracking-wider">
                Live Workout Session
              </span>
              <span className="text-[#8C8278]">•</span>
              <span className="text-xs font-mono-num text-[#211D1B] flex items-center gap-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {activeSplit && activeSplit.days.length > 0 && (
              <select
                ...
                className="flex-1 sm:flex-none bg-[#ECE7DC] border border-[#D4CBC0] text-xs font-mono-num text-[#211D1B] rounded-lg px-2.5 py-2 focus:outline-none focus:border-[#781D2E] min-h-[40px]"
              >
            ...
            <div className="flex items-center bg-[#ECE7DC] p-1 rounded-lg border border-[#D4CBC0] text-xs font-mono-num">
              <button
                ...
                className={`px-2.5 py-1 rounded transition ${
                  weightUnit === 'lbs' ? 'bg-[#781D2E] text-[#FAF8F5] font-bold' : 'text-[#6B635B]'
                }`}
              >
                LBS
              </button>
              <button
                ...
                className={`px-2.5 py-1 rounded transition ${
                  weightUnit === 'kg' ? 'bg-[#781D2E] text-[#FAF8F5] font-bold' : 'text-[#6B635B]'
                }`}
              >
                KG
              </button>
            ...
            <button
              ...
              disabled={isSaving}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold bg-[#781D2E] hover:bg-[#5C1221] active:scale-98 text-[#FAF8F5] shadow-xs transition disabled:opacity-50 min-h-[40px]"
            >
        ...
        <div className="space-y-1 pt-1 border-t border-[#E3DCD1]">
          <div className="flex justify-between text-[11px] font-mono-num text-[#6B635B]">
            <span>
              Progress: <strong className="text-[#211D1B]">{completedSetsCount}</strong> of {totalPlannedSets} sets completed
            </span>
          ...
          <div className="w-full bg-[#ECE7DC] h-1.5 rounded-full overflow-hidden">
            <div
              className="bg-[#781D2E] h-full transition-all duration-300 rounded-full"
              style={{
      ...
      <div className="bg-[#FAF8F5] border border-[#E3DCD1] p-3 sm:p-4 rounded-xl shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#182333] text-[#FAF8F5] flex items-center justify-center">
            <Timer className="w-4 h-4" />
          </div>
          <div>
            <div className="text-[9px] uppercase font-mono-num font-bold text-[#6B635B] tracking-wider">
              Rest Countdown
            </div>
            <div className="text-base sm:text-lg font-mono-num font-bold text-[#211D1B] flex items-center gap-2">
              <span>{restTimeLeft !== null ? formatTimer(restTimeLeft) : formatTimer(restInitialTime)}</span>
              {isRestTimerActive && (
                <span className="text-[10px] text-[#781D2E] uppercase font-mono-num font-bold animate-pulse">
                  Resting
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-xs font-mono-num font-medium transition min-h-[36px] ${
                restInitialTime === seconds && isRestTimerActive
                  ? 'bg-[#182333] text-[#FAF8F5] font-bold'
          ...
          {isRestTimerActive ? (
            <button
              ...
              className="px-3 py-1.5 bg-[#781D2E]/10 hover:bg-[#781D2E]/20 text-[#781D2E] border border-[#781D2E]/30 rounded-lg text-xs font-mono-num font-semibold transition min-h-[36px]"
            >
              Stop
            </button>
          ) : (
            <button
              ...
              className="px-3 py-1.5 bg-[#182333] text-[#FAF8F5] rounded-lg text-xs font-mono-num font-semibold transition min-h-[36px]"
            >
              Start
            </button>
          )}
      ...
      {Object.keys(liveMuscleCounts).length > 0 && (
        <div className="bg-[#ECE7DC]/60 border border-[#D4CBC0] p-3 rounded-xl space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(liveMuscleCounts).map(([muscle, counts]) => (
              <span
                key={muscle}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs bg-[#FAF8F5] border border-[#D4CBC0] text-[#211D1B] font-mono-num"
              >
                <strong className="text-[#781D2E]">{muscle}:</strong>
                <span>{counts.primary} sets</span>
                {counts.secondary > 0 && (
                  <span className="text-[#8C8278] text-[10px]">(+{counts.secondary} sec)</span>
                )}
              </span>
            ))}
          </div>
      ...
      <div className="space-y-3.5">
        {exercises.map((ex, exIdx) => {
          ...
          return (
            <div
              key={ex.id}
              className="bg-[#FAF8F5] border border-[#E3DCD1] rounded-xl p-3.5 sm:p-5 space-y-3 shadow-xs"
            >
              {/* Exercise Header */}
              <div className="flex items-start justify-between gap-2 pb-2.5 border-b border-[#E3DCD1]">
                <div className="flex items-start gap-2.5">
                  <span className="w-6 h-6 rounded-md bg-[#781D2E] text-[#FAF8F5] text-xs font-mono-num font-bold flex items-center justify-center shrink-0 mt-0.5">
                    {exIdx + 1}
                  </span>
                  <div>
                    <h3 className="font-bold text-[#211D1B] text-sm sm:text-base">{ex.exerciseName}</h3>
                    <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-xs font-mono-num">
                      <span className="bg-[#ECE7DC] text-[#211D1B] px-1.5 py-0.2 rounded text-[10px] font-semibold border border-[#D4CBC0]">
                        {ex.primaryMuscle}
                      </span>
                      {ex.targetRepsRaw && (
                        <span className="text-[#6B635B] text-[11px]">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono-num text-[#6B635B]">
                    {completedExSets}/{ex.sets.length}
                  </span>
                  <button
                    onClick={() => removeExercise(exIdx)}
                    title="Remove movement"
                    className="p-1.5 text-[#8C8278] hover:text-[#781D2E] rounded-md transition"
                  >
                ...
                {ex.sets.map((set, sIdx) => (
                  <div
                    ...
                    className={`p-2.5 sm:p-3 rounded-lg border transition flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 ${
                      set.completed
                        ? 'bg-[#781D2E]/5 border-[#781D2E]/30'
                        : 'bg-[#ECE7DC]/30 border-[#D4CBC0]/80'
                    }`}
                  >
                    {/* Set Label */}
                    <div className="flex items-center justify-between sm:w-16">
                      <span className="text-xs font-mono-num font-bold text-[#6B635B]">
                        SET {sIdx + 1}
                      </span>
                      ...
                      {ex.sets.length > 1 && (
                        <button
                          ...
                          className="sm:hidden text-[#8C8278] hover:text-[#781D2E] p-1"
                        >
                    ...
                    {/* Weight & Reps Stepper Controls (Super Phone-Friendly) */}
                    <div className="flex items-center gap-3 sm:gap-4 flex-1">
                      {/* Weight Control */}
                      <div className="flex-1 space-y-1">
                        <span className="text-[9px] uppercase font-mono-num font-semibold text-[#8C8278] block">
                          Weight ({weightUnit.toUpperCase()})
                        </span>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => adjustNumericValue(exIdx, sIdx, 'weight', -5)}
                            className="w-7 h-8 bg-[#ECE7DC] hover:bg-[#D4CBC0] text-[#211D1B] rounded font-mono-num font-bold text-xs flex items-center justify-center border border-[#D4CBC0] active:scale-95"
                          >
                            -5
                          </button>
                          <input
                            ...
                            className="w-16 sm:w-20 bg-[#FAF8F5] border border-[#D4CBC0] focus:border-[#781D2E] rounded px-2 py-1 text-center font-mono-num font-bold text-xs sm:text-sm text-[#211D1B] focus:outline-none"
                          />
                          <button
                            onClick={() => adjustNumericValue(exIdx, sIdx, 'weight', 5)}
                            className="w-7 h-8 bg-[#ECE7DC] hover:bg-[#D4CBC0] text-[#211D1B] rounded font-mono-num font-bold text-xs flex items-center justify-center border border-[#D4CBC0] active:scale-95"
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            ...
                            className="w-14 sm:w-16 bg-[#FAF8F5] border border-[#D4CBC0] focus:border-[#781D2E] rounded px-2 py-1 text-center font-mono-num font-bold text-xs sm:text-sm text-[#211D1B] focus:outline-none"
                          />
                          <button
                            onClick={() => adjustNumericValue(exIdx, sIdx, 'reps', 1)}
                            className="w-7 h-8 bg-[#ECE7DC] hover:bg-[#D4CBC0] text-[#211D1B] rounded font-mono-num font-bold text-xs flex items-center justify-center border border-[#D4CBC0] active:scale-95"
                      ...
                      <div className="w-16 space-y-1 hidden sm:block">
                        ...
                        <select
                          ...
                          className="w-full bg-[#FAF8F5] border border-[#D4CBC0] rounded px-1.5 py-1 text-xs font-mono-num text-[#211D1B] focus:outline-none focus:border-[#781D2E]"
                        >
                    ...
                    <div className="flex items-center justify-end gap-2 pt-1 sm:pt-0">
                      <button
                        ...
                        className={`flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 sm:py-1.5 rounded-lg text-xs font-mono-num font-bold transition min-h-[44px] sm:min-h-[36px] active:scale-98 ${
                          set.completed
                            ...
                            : 'bg-[#ECE7DC] hover:bg-[#D4CBC0] text-[#6B635B] border border-[#D4CBC0]'
                        }`}
                      >
                      ...
                      {ex.sets.length > 1 && (
                        <button
                          onClick={() => removeSetFromExercise(exIdx, sIdx)}
                          className="hidden sm:block text-[#8C8278] hover:text-[#781D2E] p-1.5 rounded"
                          title="Remove set"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
              ...
              <button
                ...
                className="w-full py-2 rounded-lg border border-dashed border-[#D4CBC0] hover:border-[#781D2E] text-[#6B635B] hover:text-[#781D2E] text-xs font-mono-num font-medium flex items-center justify-center gap-1.5 transition min-h-[40px]"
              >
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch">
        <button
          ...
          className="py-3 px-4 rounded-xl border border-[#D4CBC0] bg-[#ECE7DC] hover:bg-[#E3DCD1] text-[#211D1B] text-xs font-mono-num font-semibold flex items-center justify-center gap-2 transition min-h-[44px]"
        >
          <Plus className="w-4 h-4 text-[#781D2E]" />
        ...
        <div className="flex-1">
          <input
            ...
            placeholder="Session notes (e.g. good lockout, increased load on press)..."
            className="w-full bg-[#FAF8F5] border border-[#D4CBC0] focus:border-[#781D2E] rounded-xl px-3.5 py-2.5 text-xs text-[#211D1B] placeholder-[#8C8278] focus:outline-none min-h-[44px]"
          />
        </div>
      </div>
      ...
      <div className="pt-2 flex items-center justify-between gap-3">
        <button
          onClick={onCancel}
          className="px-4 py-2.5 rounded-lg text-xs font-mono-num font-medium text-[#6B635B] hover:text-[#211D1B] hover:bg-[#ECE7DC] transition min-h-[44px]"
        >
          Cancel
        </button>
        ...
        <button
          onClick={handleFinishWorkout}
          disabled={isSaving}
          className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs sm:text-sm font-mono-num font-bold bg-[#781D2E] hover:bg-[#5C1221] active:scale-98 text-[#FAF8F5] shadow-sm transition disabled:opacity-50 min-h-[44px]"
        >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-[#211D1B]/50 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-[#FAF8F5] border-t sm:border border-[#E3DCD1] rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-200">
            <div className="p-4 border-b border-[#E3DCD1] flex items-center justify-between">
              <h3 className="font-serif-title font-bold text-[#211D1B] text-sm sm:text-base">
                Select Exercise
              </h3>
              <button
                onClick={() => setShowAddModal(false)}
                className="text-[#6B635B] hover:text-[#211D1B] p-1.5"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="p-3 border-b border-[#E3DCD1]">
              <div className="relative">
                <Search className="w-4 h-4 text-[#8C8278] absolute left-3 top-1/2 -translate-y-1/2" />
                <input
                  type="text"
                  ...
                  autoFocus
                  className="w-full bg-[#ECE7DC]/60 border border-[#D4CBC0] focus:border-[#781D2E] focus:bg-[#FAF8F5] rounded-lg pl-9 pr-3 py-2 text-xs text-[#211D1B] placeholder-[#8C8278] focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="overflow-y-auto p-3 space-y-1.5 flex-1 max-h-96">
              {filteredSearchExercises.length === 0 ? (
                <div className="p-6 text-center text-xs text-[#8C8278] font-mono-num">
                ...
                filteredSearchExercises.map((ex) => (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-xl bg-[#ECE7DC]/40 hover:bg-[#ECE7DC] border border-[#D4CBC0] flex items-center justify-between text-xs transition active:scale-98 min-h-[48px]"
                  >
                    <div>
                      <div className="font-bold text-[#211D1B]">{ex.name}</div>
                      <div className="text-[11px] text-[#6B635B] font-mono-num mt-0.5">
                        Primary: <span className="text-[#781D2E] font-semibold">{ex.primaryMuscle}</span>
                        {ex.secondaryMuscles?.length > 0 && (
                          <span className="text-[#8C8278]"> ({ex.secondaryMuscles.join(', ')})</span>
                        )}
                      </div>
                    </div>
                    <span className="text-[10px] font-mono-num font-semibold bg-[#FAF8F5] text-[#211D1B] px-2 py-0.5 rounded border border-[#D4CBC0]">
                      {ex.category}
                    </span>
                  </button>
                ))
